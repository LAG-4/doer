...
Linked worktrees default to their own `.t3/userdata`, even when `DOER_HOME` or `T3CODE_HOME` is set.
The main checkout defaults to `~/.doer/dev/userdata`. An explicit `--home-dir` wins in both cases.
Never run a development server against the live `~/.doer/userdata`.
