...
stdout/stderr at `~/.doer/ssh-launch/<state>/server.log`.
...
- SSH-managed launch persistence: `~/.doer/ssh-launch/<state>/server.log`
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.doer/userdata/...` by default, or
...
worktree uses `~/.doer/dev/logs/server.trace.ndjson`.
...
```bash
TRACE_FILE="${DOER_HOME:-${T3CODE_HOME:-$HOME/.doer}}/userdata/logs/server.trace.ndjson"
```
...
```bash
TRACE_FILE="$HOME/.doer/dev/logs/server.trace.ndjson"
```
