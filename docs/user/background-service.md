...
Start with `doer service status` on the host. It prints the log path and, on Linux,
...
identity. Without administrator access, run `doer serve` in a terminal and keep
that session open.
...
| `service-disabled` or `service-stopped` | Read the log and `systemctl --user status doer.service`, then use the repair command printed by Doer.                          |
