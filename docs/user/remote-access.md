...
On a command-line host, `doer connect unlink` disables exposure while retaining
your login; `doer connect logout` also clears that login. Background-service
...
Run `doer connect status` on the host to inspect saved authorization and link
configuration. It is not a live reachability check. If the environment appears
offline, run `doer service status` and read the displayed log. If it disappears
...
| `auth_invalid` or `invalid_bearer`                        | Run `doer connect login`. If credentials were revoked, run `doer connect logout`, then `doer connect` again. Restart the server after signing in. |
...
`systemctl --user restart doer.service` for the background service. For a
foreground server, stop it and run `doer serve` again with your usual options.
