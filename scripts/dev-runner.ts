...
export const DEFAULT_T3_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  // Doer dev default is independent from T3 Code's ~/.t3.
  path.join(NodeOS.homedir(), ".doer"),
);
...
const MODE_ARGS = {
  dev: [
    ...
    "--filter=@t3tools/web",
    "--filter=@lag4/doer-cli",
    "--parallel",
    "dev",
  ],
  "dev:server": ["run", "--filter=@lag4/doer-cli", "dev"],
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .t3 > ambient DOER_HOME/T3CODE_HOME)
    // is resolved by the caller; an unset t3Home here genuinely means "use the default".
    ...
    if (configuredBaseDir !== undefined) {
      // Set both so old (T3CODE_HOME-only) and new (DOER_HOME-first)
      // servers resolve the same explicit home.
      output.DOER_HOME = resolvedBaseDir;
      output.T3CODE_HOME = resolvedBaseDir;
    } else {
      delete output.DOER_HOME;
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
    ...
    // outrank an ambient DOER_HOME/T3CODE_HOME. `--home-dir` still wins.
    ...
    const resolvedT3Home =
      ...
      worktreeHome ??
      (hostEnvironment.DOER_HOME?.trim() || hostEnvironment.T3CODE_HOME?.trim() || undefined);
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.DOER_HOME ?? env.T3CODE_HOME ?? (yield* DEFAULT_T3_HOME);
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.String("home-dir").pipe(
    Flag.withDescription(
      "Explicit Doer data directory; runtime state is stored under userdata (equivalent to DOER_HOME, with T3CODE_HOME as fallback). Inside a git worktree this defaults to that worktree's own .t3 so dev state stays off the shared home.",
    ),
    Flag.optional,
