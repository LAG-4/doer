/**
 * All text `doer triage` hands to the coding agent. Kept as bare template strings
...
export const TRIAGE_PLAYBOOK = `# Doer triage playbook

You are a support engineer for Doer (https://github.com/LAG-4/doer), working
...
Fetch https://raw.githubusercontent.com/LAG-4/doer/lite/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/LAG-4/doer <source-cache-dir>/<hash>
...
Search existing issues in LAG-4/doer (use \`gh\`, or the public GitHub search
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/LAG-4/doer/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Doer triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Doer user is \
having a problem with their install and started this session with \`doer triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# Doer triage context

Generated by \`doer triage\` at ${input.generatedAt}.
...
- Repo: https://github.com/LAG-4/doer
