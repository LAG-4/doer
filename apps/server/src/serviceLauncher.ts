...
// `doer service update`. The launcher supervises the server child for the boot
// service and must keep working across server versions, so it stays on Node
// built-ins with no Effect runtime: it is the one part of the executable that
// cannot depend on the rest of it being loadable.
...
const runtimePaths = (baseDir: string, version: string) => {
  ...
  return {
    versionDir,
    entryPath: NodePath.join(versionDir, "node_modules", "@lag4/doer-cli", "dist", "bin.mjs"),
...
export class Launcher {
  ...
  async #startChild(version: string, role: ChildRole, update?: ServiceUpdateRecord): Promise<void> {
    ...
    if (!(await runtimeExists(this.#baseDir, version))) {
      throw new Error(`Selected @lag4/doer-cli@${version} runtime is missing or incomplete.`);
    }
    if (this.#stopping) return;
