...
function configuredMcpToolAvailability(
  ...
  if (!hasConfiguredMcpServer(appServerArgs))
    return { browser: false, device: false, computer: false };
  // Callers predating the capability set attached the browser toolkit only.
  if (mcpCapabilities === undefined) return { browser: true, device: false, computer: false };
  return {
    browser: mcpCapabilities.has("preview"),
    device: mcpCapabilities.has("device"),
    computer: mcpCapabilities.has("computer"),
  };
}
