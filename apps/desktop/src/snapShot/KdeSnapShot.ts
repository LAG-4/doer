...
const DESKTOP_FILE = "click.lagaryan.Doer.KdeCapture.desktop";
const MARKER = "X-Doer-Capture-Helper=true";
...
export function kdeCapturePaths(paths: KdeCapturePaths) {
  return {
    executable: NodePath.join(paths.dataHome, "doer", "kde-capture", KDE_CAPTURE_EXECUTABLE),
...
export class KdeCaptureSetup {
  ...
  async state(): Promise<DesktopCaptureHelperState> {
    try {
      ...
      if (!bundle)
        return {
          status: "error",
          message: "The capture helper is missing from this build. Update or reinstall Doer.",
        };
  ...
  async perform(action: "install-kde-helper" | "remove-kde-helper") {
    ...
    if (action === "remove-kde-helper") {
      ...
      const bundle = await regularFile(this.paths.bundle);
      if (!bundle)
        throw new Error("The capture helper is missing from this build. Update or reinstall Doer.");
